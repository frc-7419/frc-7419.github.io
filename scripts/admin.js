...
async function import_gdoc() {
  ...
  fs.readdirSync(BASEDIR).forEach((file) => {
    ...
    if (
      fs.statSync(path.join(BASEDIR, file)).isFile() &&
      file.split(".").pop() == "zip"
    ) {
      zips.push(file);
    }
  ...
  if (zips.length == 0) {
    console.error(`No zip files found in ${BASEDIR}, aborting`);
    return;
  }
  let zip;
  if (zips.length == 1) {
  ...
  if (fs.existsSync(TMPIMGDIR)) {
    ...
    if (fs.existsSync(IMGDIR)) {
      console.error(`${IMGDIR} already exists, aborting`);
    }
  ...
  fs.readdirSync(TMPDIR).forEach((file) => {
    // if is file and ends in `.zip`
    if (
      fs.statSync(path.join(TMPDIR, file)).isFile() &&
      file.split(".").pop() == "html"
    ) {
      HTMLFILE = file;
    }
